...
def checkByDict(word_l, word):
  ...
  elif word_l in dct['conjunction']: word['POSpeech'] = 'conjunction'      # союз
...
def _getMorphA(word):
  ...
  # (для неизменяемых или почти неизменяемых частей речи)
  if checkByDict(word_l, word): return GN
  ...
  #ERROR слово prezenten и enden определяется наречием. Другие слова на -n могут ошибочно определиться.
    ...
    GN.extend(_getMorphA(temp_word2))
...
def getMorphA(sentence):
  ...
  for word in sentence.getSentence('listDict'):
    GrammarNazi.extend(_getMorphA(word))
  #pr(sentence.getSentence('listDict'))
